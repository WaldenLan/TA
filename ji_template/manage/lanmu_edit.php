	...
	<div id="mmain" class="mmain">
		...
		<form action="../lanmu_save" method="post">
			<table width="800" border="0" class="page_add" cellpadding="8" cellspacing="1" bgcolor="#D4D4D4">
				...
				<tr<?php if (!isset($_SESSION['user']) || $_SESSION['user'] != master)
				{
					echo " style='display:none'";
				} ?>>
					<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE">
						<strong>父级栏目&nbsp;</strong></td>
					<td width="680" height="30" bgcolor="#F0F0F0"><label>
							<select name="lm_pid">
								<option <?php if ($o->lm_pid == 0)
								{ ?> selected="selected"<?php }; ?> value="0">创建为一级栏目
								</option>
								<?php foreach ($lanmu as $l): ?>
									<option <?php if ($o->lm_pid == $l->id)
									{ ?> selected="selected"<?php }; ?> value="<?= $l->id ?>"><?= $l->lm_name ?>
										[<?= $l->lm_enname ?>]
									</option>
									<?php $sublanmu =
									...
									{
										?>
										<option value="<?= $s->id ?>" <?php if ($o->lm_pid == $s->id)
										{ ?> selected="selected"<?php }; ?>>&nbsp;&nbsp;-<?= $s->lm_name ?>
											[<?= $s->lm_enname ?>]
										</option>
									<?php } ?>
								<?php endforeach ?>
				...
				<tr>
					...
					<td width="680" height="30" bgcolor="#F0F0F0"><label>
							<input type="text" name="lm_keywords" value="<?= $o->lm_keywords; ?>"/>不设置请留空，,分割</label>
					</td>
				</tr>
				<tr>
					<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE">
						<strong>SEO描述&nbsp;</strong></td>
					<td width="680" height="30" bgcolor="#F0F0F0"><label>
