<div id="mmain" class="mmain">
	...
	<form action="../file_save/<?= $file['id'] ?>" method="post" enctype="multipart/form-data">
		<table width="800" border="0" class="file_add" cellpadding="8" cellspacing="1" bgcolor="#D4D4D4">
			<tr>
				<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE"><strong>文件名称&nbsp;</strong>
				</td>
			...
			<tr>
				<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE"><strong>文件分类&nbsp;</strong>
				</td>
				<td width="680" height="30" bgcolor="#F0F0F0"><label>
						<select name="file_class">
							<?php foreach ($lanmu as $l): ?>
								<option value="<?= $l->id ?>" <?php if ($l->id == $file['file_class'])
								...
								foreach ($lr as $s):
									?>
									<option <?php if ($file['file_class'] == $s->id)
									{ ?> selected="selected"<?php } ?> value="<?= $s->id ?>">
										&nbsp;&nbsp;&nbsp;&nbsp;<?= $s->lm_name ?></option>
									<!--↓三级栏目-->
									<?php
									$lanmuthree = $this->db->query("SELECT * FROM ji_lanmu where lm_pid=" . $s->id . "")
									                       ->result();
									foreach ($lanmuthree as $lt):
										?>
										<option <?php if ($file['file_class'] == $lt->id)
										{ ?> selected="selected"<?php } ?> value="<?= $lt->id ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-<?= $lt->lm_name ?></option>
									<?php endforeach; ?>
									<!--↑三级栏目-->
								<?php endforeach; ?>
							<?php endforeach; ?>
			...
			<tr>
				<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE"><strong>语言&nbsp;</strong>
				</td>
				<td width="680" height="30" bgcolor="#F0F0F0"><label>
						<select name="file_en">
							<option value="0"<?php if ($file['file_en'] == 0)
							{ ?> selected="selected"<?php } ?>>中文
							</option>
							<option value="1"<?php if ($file['file_en'] == 1)
			...
			<tr>
				<td width="120" height="30" align="right" valign="middle" bgcolor="#CFE2FE"><strong>上传时间&nbsp;</strong>
				</td>
				<td width="680" height="30" bgcolor="#F0F0F0"><label>
			...
			<tr>
				...
				<td width="680" height="30" bgcolor="#F0F0F0"><label>
						<input type="checkbox" name="file_publish"<?php if ($file['file_publish'] == 1)
