...
if ($rs2['student_major'] != $ad['advisor_major'])
{
	echo "<body><div class='warning'>Warning: You are not in the " . $ad['advisor_major'] . " list.  " .
	     "<a href='index.php'>Back</a></div></body>";
	die;
}

if ($rs2['student_status'] == 1)
{//如果允许该学生选老师
	
	if ($id != $rs['id'])
	{//如果获取到的身份证ID和数据库中的不一样
		if ($rs['id'] != '')
		{
			echo "<body><div class='warning'>Warning: Wrong!Please try it again.  " .
			     "<a href='index.php'>Back</a></div></body>";
		}
	}
	else
	{
		if ($rs2['advisor_id'] != '0')
		{
			...
			{
				$sqladvisor =
						"update `advisor` set advisor_count=`advisor_count`-1 where advisor_id=" . $rs2['advisor_id'] .
						"";
				mysql_query($sqladvisor, $conn);//计数加1
				$sqladvisor2 =
						"update `advisor` set advisor_count=`advisor_count`+1 where advisor_id=" . $advisor_id . "";
		...
		$sqlstudent = "update `student` set student_name='" . $student_name . "',student_phone='" . $student_phone .
		              "',advisor_id='" . $advisor_id . "' where student_id='" . $student_id . "'";
