...
{
	function addlog($action, $time, $id, $model, $name, $institute)
	{//增加事件记录
		$this->db->query("insert into ji_user_log set `log_action`='" . $action . "',`log_time`='" . $time .
		                 "',`user_id`='" . $id . "',`log_model`='" . $model . "',`user_name`='" . $name .
	...
	{//普通用户获取自己的模块
		return $this->db->query("select * from `ji_user_permission` where user_id='" . $_SESSION['user'] . "'")
		                ->result();
	}
