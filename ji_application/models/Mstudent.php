...
class Mstudent extends CI_Model
{
	...
	{
		$query =
			$this->db->select('BSID')->from('ji_course_select')->where(array('USER_ID' => $id, 'SCBJ' => 'N'))->get();
		return $query->result();
	}
	...
	public function get_now_course($id)
	{
		...
		$query = $this->db->select('*')->from('ji_course_open')->where(array(
			                                                               'XQ' => $site_config['ji_academic_term'],
			                                                               'XN' => $site_config['ji_academic_year'],
			                                                               'SCBJ' => 'N'
		                                                               ))->where_in('BSID', $course_list)->get();
