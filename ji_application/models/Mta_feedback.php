...
class Mta_feedback extends CI_Model
{
	...
	public function get_state_str($id)
	{
		...
		{
		case 0 :
			return 'applying to manager';
		case 1 :
			return 'disposed by manager';
		case 2 :
			return 'applying to teacher';
		case 3 :
			return 'disposed by teacher';
		case -1:
			return 'cancelled';
		default:
	...
	/**
	 * 老师显示投诉TA申请列表
	 * @param   $teacher_id  string  老师 ID
	 * @param   $state       int     状态（2：未处理 3：已处理）
	 * @return  array[][]
	 */
	...
	public function manage_add_feedback($data)
	{
		...
		{
			// 管理员可以处理，回复学生邮件
			$this->db->update('ji_ta_feedback', array(
				'state'           => 1,
				'manage_reply_id' => $this->db->insert_id()
			), array('id' => $data['id']));
	...
	public function teacher_add_feedback($data)
	{
		...
		$this->db->insert('ji_ta_feedback_reply', array('user_id' => $data['user_id'], 'content' => $data['content']));
		$this->db->update('ji_ta_feedback', array(
			'state' => 3, 'teacher_reply_id' => $this->db->insert_id()
		), array('id' => $data['id']));
	}
