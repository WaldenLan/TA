...
class Feedback extends TA_Controller
{
	...
	public function view()
	{
		...
		$current_page =
			floor(min(max($this->uri->segment(6), 1), ($config['total_rows'] - 1) / $config['per_page'] + 1));
		...
		foreach ($data['course_list'] as $course)
		{
			if ($course->BSID == $form_data['BSID'])
			{
				foreach ($course->ta_list as $ta)
				{
					...
					{
						if (strlen($form_data['content']) >= 10 && strlen($form_data['title']) >= 5 &&
						    strlen($form_data['title']) <= 20
						)
						{
