...
//			echo $this->upload->display_errors(); 输出错误信息
			...
			if ($this->input->post('id'))
			{
				...
				$this->Mmanage->addlog('更新ji_slide的id为' . $this->input->post('id') .
				                       '的信息', now, $_SESSION['user'], 'slide', $_SESSION['user'], 'JI');
			}
			else
			{
				$this->db->insert('ji_slide', $slidesave);
				$this->Mmanage->addlog('增加ji_slide的id为' . mysql_insert_id() .
				                       '的信息', now, $_SESSION['user'], 'slide', $_SESSION['user'], 'JI');
			}
	...
	function lanmu_save()
	{
		if ($this->input->post('id'))
		{
			...
			$this->Mmanage->addlog('更新ji_lanmu的id为' . $this->input->post('id') .
			                       '的信息', now, $_SESSION['user'], 'lanmu', $_SESSION['user'], 'JI');
		}
		else
		{
			$savelm = array(
			...
			$this->Mmanage->addlog('增加ji_lanmu的id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'lanmu', $_SESSION['user'], 'JI');
		};
	...
	function page_save()
	{
		...
		if (!$this->upload->do_upload('page_pic'))
		{//如果没有执行上传操作，则不对图片数据进行操作息
			$savepage = array(
			...
			if ($this->input->post('id'))
			{
				...
				$this->Mmanage->addlog('更新ji_page的id为' . $this->input->post('id') .
				                       '的信息', now, $_SESSION['user'], 'page', $_SESSION['user'], 'JI');
			}
			else
			{
				$this->db->insert('ji_page', $savepage);
				$this->Mmanage->addlog('增加ji_page的id为' . mysql_insert_id() .
				                       '的信息', now, $_SESSION['user'], 'page', $_SESSION['user'], 'JI');
			}
		}
		else
	...
	{//删除页面
		...
		$this->Mmanage->addlog('删除ji_page的id为' . $delid .
		                       '的信息', now, $_SESSION['user'], 'page', $_SESSION['user'], 'JI');
...
//			echo $this->upload->display_errors(); 输出错误信息
	...
	function file_save()
	{
		...
		if (!$this->upload->do_upload('file_file') && $id)
		{//如果没有上传成功 并且为编辑状态
			$savefile = array(
		...
		else
		{
			...
			{//如果上传了，并且不是编辑，则增加
				...
				$this->Mmanage->addlog('修改ji_file中id为' . $id .
				                       '的信息', now, $_SESSION['user'], 'file', $_SESSION['user'], 'JI');
			}
			else
			{//上传了，但是为编辑，则更新
	...
	function file_del()
	{
		...
		$this->Mmanage->addlog('删除ji_file中id为' . $this->uri->segment(3) .
		                       '的信息', now, $_SESSION['user'], 'file', $_SESSION['user'], 'JI');
	...
	function user_save()
	{
		...
		if ($password == $data['user_password'] && $pwd1 == $pwd2)
		{//如果旧用户与密码都正确，而且密码与确认密码一致
			$arr = array(
				'user_name'     => $user,
				'user_password' => $pwd1
			);
	...
	function video_save()
	{
		...
		if (!$this->upload->do_upload('v_pic'))
		{
			...
			if (!$this->input->post('id'))
			{
				...
				$this->Mmanage->addlog('增加ji_video中id为' . mysql_insert_id() .
				                       '的信息', now, $_SESSION['user'], 'video', $_SESSION['user'], 'JI');
			}
			else
			{
				$this->db->where('id', $this->input->post('id'));
				$this->db->update('ji_video', $vsave);
				$this->Mmanage->addlog('修改ji_video中id为' . $this->input->post('id') .
				                       '的信息', now, $_SESSION['user'], 'video', $_SESSION['user'], 'JI');
			}
		}
		else
	...
	{//删除幻灯片
		...
		$this->Mmanage->addlog('删除ji_video中id为' . $delid .
		                       '的信息', now, $_SESSION['user'], 'video', $_SESSION['user'], 'JI');
	...
	function tuisong_save()
	{
		...
		if ($this->input->post('tid'))
		{
			...
			$this->Mmanage->addlog('修改ji_tuisong中tid为' . $this->input->post('tid') .
			                       '的信息', now, $_SESSION['user'], 'tuisong', $_SESSION['user'], 'JI');
	...
	function tuisong_del()
	{
		...
		$this->Mmanage->addlog('删除tuisong中id为' . $delid .
		                       '的信息', now, $_SESSION['user'], 'tuisong', $_SESSION['user'], 'JI');
	...
	{//转学分
		...
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'disable')
		{//关闭大学
			$this->db->query("update equivalence_university set university_status=0 where university_id=" .
			                 $this->uri->segment(5) . "");
			header("Location:/manage/equivalence");
			die;
		}
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'enable')
		{//开启大学
			$this->db->query("update equivalence_university set university_status=1 where university_id=" .
			                 $this->uri->segment(5) . "");
			header("Location:/manage/equivalence");
			die;
		}
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'course')
		{//大学的课程
		...
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'add')
		{//添加大学
			$this->load->view("manage/equivalence_university_add.php");
		}
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'edit')
		{//编辑大学
			$data['university'] = $this->Mmanage->get_university();
			$this->load->view("manage/equivalence_university_edit.php", $data);
		}
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'save')
		{//保存大学
			...
			$this->Mmanage->addlog('增加equivalence_university中university_id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'equivalence', $_SESSION['user'], 'JI');
			header("Location:/manage/equivalence");
		}
		if ($this->uri->segment(3) == 'university' && $this->uri->segment(4) == 'edit_save')
		{//保存大学
			if (!$this->input->post('top'))
			{
				$top = 0;
			...
			$this->Mmanage->addlog('修改equivalence_university中university_id为' . $this->input->post('id') .
			                       '的信息', now, $_SESSION['user'], 'equivalence', $_SESSION['user'], 'JI');
			
			header("Location:/manage/equivalence");
		}
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'add')
		{//添加课程
			$data['universities'] = $this->Mmanage->get_universities();
			$this->load->view("manage/equivalence_course_add.php", $data);
		}
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'open')
		{//开启课程转学分
		...
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'close')
		{//关闭课程转学分
		...
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'edit')
		{//添加课程
			$data['universities'] = $this->Mmanage->get_universities();
			$data['course'] = $this->Mmanage->get_course();
			$this->load->view("manage/equivalence_course_edit.php", $data);
		}
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'save')
		{//保存课程
			if (!$this->input->post('top'))
			{
				$top = 0;
			...
			$this->Mmanage->addlog('增加equivalence_course中course_id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'equivalence', $_SESSION['user'], 'JI');
			
			header("Location:/manage/equivalence/university/course/" . $this->input->post('university') . "");
		}
		if ($this->uri->segment(3) == 'course' && $this->uri->segment(4) == 'edit_save')
		{//保存课程
			if (!$this->input->post('top'))
			{
				$top = 0;
			...
			$this->Mmanage->addlog('修改equivalence_course中course_id为' . $this->input->post('course_id') .
			                       '的信息', now, $_SESSION['user'], 'equivalence', $_SESSION['user'], 'JI');
			header("Location:/manage/equivalence/university/course/" . $this->input->post('university') . "");
		}
	}
	...
	{//保存通讯录
		...
		if (!$this->input->post('detail_id'))
		{
			...
			$this->Mmanage->addlog('增加ji_user_detail中detail_id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'contactlist', $_SESSION['user'], 'JI');
		...
		else
		{
			...
			$this->Mmanage->addlog('修改ji_user_detail中detail_id为' . $this->input->post('id') .
			                       '的信息', now, $_SESSION['user'], 'contactlist', $_SESSION['user'], 'JI');
		}
	...
	{//GPA
		...
		{//获取到年级
			$f10 =
				"'F1037101','F1037102','F1037103','F1037104','F1037105','F1037201','F1037202','F1037203','F1037204','F1037205'";
			$f11 =
				"'F1137101','F1137102','F1137103','F1137104','F1137105','F1137201','F1137202','F1137203','F1137204','F1137205'";
			$f12 =
				"'F1237101','F1237102','F1237103','F1237104','F1237105','F1237201','F1237202','F1237203','F1237204','F1237205'";
			$f13 =
				"'F1337101','F1337102','F1337103','F1337104','F1337105','F1337201','F1337202','F1337203','F1337204','F1337205'";
			$f14 =
				"'F1437101','F1437102','F1437103','F1437104','F1437105','F1437201','F1437202','F1437203','F1437204','F1437205'";
			$f15 =
				"'F1537101','F1537102','F1537103','F1537104','F1537105','F1537201','F1537202','F1537203','F1537204','F1537205'";
			...
			$data['students'] = $this->db->query("select * from gpa_student where student_class in(" . $$student_grade .
			                                     ") and student_major='" . $student_major . "'" . $tiaojian .
			                                     " order by student_id asc")->result();
	...
	{//GPA
		...
		if ($studentid)
		{
			...
			$results1 = $this->db->query("select * from (select * from `gpa_result` where student_id='" . $studentid .
			                             "' and result_ksdj<>'03'" . $tiaojian .
			                             " and result_kcdm not in('VE490','VM490','TH009','VV155','VY109','CS000') order by result_xn desc,result_xq desc) `temptable` group by result_kcdm")
			                     ->result();////获取该学生的所有成绩，剔除两级制，重复出现取最新（排除了一些例外）
			$results2 = $this->db->query("select * from `gpa_result` where student_id='" . $studentid .
			                             "' and result_ksdj<>'03'" . $tiaojian .
			                             " and result_kcdm in('VE490','VM490','TH009','VV155','VY109','CS000')")
			                     ->result();////获取该学生的所有成绩，剔除两级制，重复出现全计算(只包含例外)
			...
			$total = $total1 + $total2;
			$data['sql1'] = "select * from (select * from `gpa_result` where student_id='" . $studentid .
			                "' and result_ksdj<>'03'" . $tiaojian .
			                " and result_kcdm not in('VE490','VM490','TH009','VV155','VY109','CS000') order by result_xn desc,result_xq desc) `temptable` group by result_kcdm;";
			$data['sql2'] =
				"select * from `gpa_result` where student_id='" . $studentid . "' and result_ksdj<>'03'" . $tiaojian .
				" and result_kcdm in('VE490','VM490','TH009','VV155','VY109','CS000')";
			...
			$gpa = $count / $total;
			$checkgpa =
				$this->db->query("select * from gpa_gpa where student_id='" . $studentid . "' and gpa_xn='" . $xn .
				                 "' and gpa_xq='" . $xq . "' limit 1")->row_array();//检查是否已经有了GPA
			if ($checkgpa)
			{//如果已经有了，则更新
				if ($data['gpa'])
				{
					$this->db->query("update gpa_gpa set student_id='" . $studentid . "',gpa=" . $gpa . ",gpa_xn='" .
					                 $xn . "',gpa_xq=" . $xq . ",gpa_createtime='" . date('y-m-d h:i:s', time()) .
					                 "' where gpa_id=" . $checkgpa['gpa_id'] . "");
				}
			}
			else
			{//如果没有记录，则重新生成
				if ($data['gpa'])
				{
					$this->db->query("insert into gpa_gpa set student_id='" . $studentid . "',gpa=" . $data['gpa'] .
					                 ",gpa_xn='" . $xn . "',gpa_xq=" . $xq . ",gpa_createtime='" .
	...
	{//GPA
		...
		{//
			...
			$end = $start + 500;
			$students = $this->db->query("select * from gpa_student where student_class in(" . $$student_grade .
			                             ") order by id asc limit " . $start . ",500")->result();
			//
			foreach ($students as $s)
			{
				...
				$results1 =
					$this->db->query("select * from (select * from `gpa_result` where student_id='" . $studentid .
					                 "' and result_ksdj<>'03'" . $tiaojian .
					                 " and result_kcdm not in('VE490','VM490','TH009','VV155','VY109','CS000') order by result_xn desc,result_xq desc) `temptable` group by result_kcdm")
					         ->result();////获取该学生的所有成绩，剔除两级制，重复出现取最新（排除了一些例外）
				$results2 = $this->db->query("select * from `gpa_result` where student_id='" . $studentid .
				...
				{//如果已经有了，则更新
					if ($gpa)
					{
						$this->db->query("update gpa_gpa set student_id='" . $studentid . "',gpa=" . $gpa .
						                 ",gpa_xn='" . $xn . "',gpa_xq=" . $xq . ",gpa_createtime='" .
						                 date('y-m-d h:i:s', time()) . "' where gpa_id=" . $checkgpa['gpa_id'] . "");
				...
				{//如果没有记录，则重新生成
					if ($gpa)
					{
						$this->db->query("insert into gpa_gpa set student_id='" . $studentid . "',gpa=" . $gpa .
						                 ",gpa_xn='" . $xn . "',gpa_xq='" . $xq . "',gpa_createtime='" .
						                 date('y-m-d h:i:s', time()) . "'");
			...
			$data['student_grade'] = $student_grade;
			$data['notice'] =
				'已经生成 ' . $student_grade . ' 的第 ' . $xn . ' 学年的第  ' . $xq . ' 学期GPA第' . $start . ' - ' . $end .
				' 个<br />';
...
//      $config['full_tag_close'] = '</div>'; // 分页结束样式
	...
	function lab_equipment_recycle()
	{//实验室设备回收站列表
		$my['module'] = 'lab';
	...
	function lab_equipment_edit()
	{//编辑设备,/manage/lab_equipment_edit/22
		$my['module'] = 'lab';
		$data['lanmu'] = $this->Mmanage->get_lab_lanmu();
	...
	function lab_equipment_del()
	{//删除设备,/manage/lab_equipment_del/22
		$my['module'] = 'lab';
		$this->load->view('manage/header', $my);
		$this->db->query("update lab_equipment set equipment_status=0 where equipment_id=" . $this->uri->segment(3) .
		                 "");
		$this->Mmanage->addlog('删除lab_equipment中equipment_id为' . $this->uri->segment(3) .
		                       '的信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		header("Location:/manage/lab");
	}
	
	function lab_equipment_back()
	{//恢复设备,/manage/lab_equipment_back/22
		$my['module'] = 'lab';
		$this->load->view('manage/header', $my);
		$this->db->query("update lab_equipment set equipment_status=1 where equipment_id=" . $this->uri->segment(3) .
		                 "");
		$this->Mmanage->addlog('恢复lab_equipment中equipment_id为' . $this->uri->segment(3) .
		                       '的信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		header("Location:/manage/lab");
	}
	
	function lab_equipment_save()
	{//保存设备
		...
		if ($this->input->post('equipment_id'))
		{
			...
			$this->Mmanage->addlog('修改设备ID为' . $this->input->post('equipment_id') .
			                       '信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		}
		else
		{
			$this->db->insert('lab_equipment', $data);
			$this->Mmanage->addlog('增加lab_equipment中equipment_id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		}
	...
	function lab_equipment_borrow_list()
	{//实验室设备借出列表
		$my['module'] = 'lab';
	...
	function lab_equipment_borrow_add()
	{//实验室设备借出添加
		$my['module'] = 'lab';
		$data['equipment'] = $this->Mmanage->get_lab_equipment();
		$this->load->view('manage/header', $my);
	...
	function lab_equipment_borrow_edit()
	{//实验室设备借出编辑
		$my['module'] = 'lab';
	...
	function lab_equipment_borrow_save()
	{//保存借用设备信息
		$data = array(
			'equipment_name'      => $this->input->post('equipment_name'),
		...
		if ($this->input->post('borrow_id'))
		{
			...
			$this->Mmanage->addlog('修改lab_equipment_borrow中borrow_id为' . $this->input->post('borrow_id') .
			                       '的信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		}
		else
		{
			$this->db->insert('lab_equipment_borrow', $data);
			$this->Mmanage->addlog('增加lab_equipment_borrow中borrow_id为' . mysql_insert_id() .
			                       '的信息', now, $_SESSION['user'], 'lab', $_SESSION['user'], 'JI');
		}
