...
class Login extends CI_Controller
{
	...
	{
		if (isset($_SESSION['jaccount']))
		{//sjtu账户认证
			$getuser = $this->db->query("select * from ji_user where user_id='" . $_SESSION['jaccount']['id'] .
			                            "' and user_status=1")->result();
			if ($getuser)
			{
	...
	{//验证用户
		...
		{
			$this->Mlogin->addlog('失败后台登陆', now, '名字：' . $user . '密码：' .
			                                     $pwd, 'manage', $user, $_SERVER["REMOTE_ADDR"]);
