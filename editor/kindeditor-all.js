...
(function (window, undefined)
{
	...
	_extend(KRange, {
		...
		shrink: function ()
		{
			...
			while (self.startContainer.nodeType == 1 && (child = self.startContainer.childNodes[self.startOffset]) && child.nodeType == 1 && !K(child)
				.isSingle())
			{
			...
			while (self.endContainer.nodeType == 1 && self.endOffset > 0 && (child = self.endContainer.childNodes[self.endOffset - 1]) && child.nodeType == 1 && !K(child)
				.isSingle())
			{
		...
		select: function (hasDummy)
		{
			...
			if (hasDummy && sc.nodeType == 1 && range.collapsed)
			{
				...
				if (_WEBKIT)
				{
					...
					if (K(sc).isInline() || so > 0 && K(children[so - 1]).isInline() || children[so] && K(children[so])
							.isInline())
					{
	...
	_extend(KEdit, KWidget, {
		init: function (options)
		{
			...
			self.iframe = K('<iframe class="ke-edit-iframe" hidefocus="true" frameborder="0"' + iframeSrc + '></iframe>')
				.css('width', '100%');
		...
		edit: function ()
		{
			...
			var dialog = self.createDialog({
				                               ...
				                               yesBtn: {
					                               ...
					                               click: function (e)
					                               {
						                               self.insertHtml('<a name="' + nameBox.val() + '">').hideDialog()
						                                   .focus();
					                               }
				                               }
			                               });
...
KindEditor.plugin('clearhtml', function (K)
{
	...
	self.clickToolbar(name, function ()
	{
		...
		html = K.formatHtml(html, {
			a: ['href', 'target'],
			embed: [
				'src', 'width', 'height', 'type', 'loop', 'autostart', 'quality', '.width', '.height', 'align',
				'allowscriptaccess'
			],
